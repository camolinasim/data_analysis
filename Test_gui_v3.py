...
# needed for new read_pcap() function
...
def populate_table_0_with_filtered_data(self, num_rows):
    print("############populating table 0 with filtered data########")
    for i in range(num_rows):
        for j in range(len(self.filtered_data.columns)):
            self.table_view.setItem(
                i, j, QTableWidgetItem(str(self.filtered_data.iat[i, j])))
...
def filter_and_show_contents_on_curent_window(self):
    ...
    output_csv_from_filtered_data = path_of_selected_pcap_no_extension + \
        'csv_folder' + '\\' + '_FILTERED.csv'
    ...
    # clear table before puting filtered data
    clear_table_view(self)

    self.filtered_data = pd.read_csv(
        csv_path, engine='python', error_bad_lines=False)
    # print(self.all_data.to_string(index=False))
    ############## HOPE FOR TABLE #####################
    # self.all_data = csv.DictReader(open(csv_path))
    table_setup(self)
    num_rows = len(self.filtered_data.index)
    ...
    for row in range(len(rows)):
        ...
        if row != 0:
            ...
            if(index_of_tab_to_show_the_filtered_data_on == 0):
                print('inside of correct if')
                set_tab_title(self, tab_index_to_edit)
                populate_table_0_with_filtered_data(self, num_rows)
...
class DataAnalysisWindow(QWidget):
    ...
    def read_pcap(self):
        ...
        # Reading pcap with scapy
        packets = rdpcap(path_of_selected_pcap)

        # ~~~ PROTOCOL STATISTICS ~~~~
        # self.label_protocol_statistics.setText(str(packets))
        protocol_statistics = str(packets)
        protocol_statistics = protocol_statistics.split()
        for stat in protocol_statistics:
            print(stat)
            stat_label = QLabel(stat)
            self.stats_frame.addWidget(stat_label)

        # temp list to hold info
        pktsList = []
        num_rows = 0
        p_len = ""
        ip_src = ""
        ...
        self.all_data = pd.DataFrame(columns=(
            'frame.number', 'time', 'ip.src', 'ip.dst', 'ip.proto', 'frame.len', 'info'))
        ...
        for pkt in packets:
            try:
                ...
                pass
            self.all_data.loc[num_rows] = [
                num_rows, pkt.time, ip_src, ip_dst, proto, p_len, info]
            num_rows += 1
        ...
        # print("########DIR########")
        # dir(frame)
        # print(f'######frame source: {frame.fields}')
        # print("#######frame destination:" + frame.dst)
